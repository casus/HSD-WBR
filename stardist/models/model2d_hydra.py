...
from stardist.src.blocks_hydra import unet_up_block, unet_down_block, ActivityRegularizationLayer
from csbdeep.utils import _raise, backend_channels_last, axes_check_and_normalize, axes_dict
from stardist.src.utils.tf import keras_import, IS_TF_1, CARETensorBoard, CARETensorBoardImage
...
from stardist.models.base_hydra import StarDistBase, _tf_version_at_least, StarDistDataBase, masked_loss_wbr
...
class StarDistData2D(StarDistDataBase):
    ...
    def __init__(self, X, Y1, Y2, batch_size, n_rays, length,
        ...
        super().__init__(X=X, Y1=Y1, Y2=Y2, n_rays=n_rays, grid=grid,
    ...
    def __getitem__(self, i):
        idx = self.batch(i)
        arrays = [sample_patches((self.Y1[k],) + (self.Y2[k],) + self.channels_as_tuple(self.X[k]),
                                 patch_size=self.patch_size, n_samples=1,
                                 valid_inds=self.get_valid_inds(k)) for k in idx]

        if self.n_channel is None:
            X, Y1, Y2 = list(zip(*[(x[0][self.b],y2[0],y1[0]) for y1,y2,x in arrays]))
        else:
            X, Y1, Y2 = list(zip(*[(np.stack([_x[0] for _x in x],axis=-1)[self.b], y2[0], y1[0]) for y1,y2,*x in arrays]))

        X, Y1, Y2 = tuple(zip(*tuple(self.augmenter(_x, _y1, _y2) for _x, _y1, _y2 in zip(X,Y1,Y2))))


        prob1 = np.stack([edt_prob(lbl[self.b][self.ss_grid[1:3]]) for lbl in Y1])
        prob2 = np.stack([edt_prob(lbl[self.b][self.ss_grid[1:3]]) for lbl in Y2])

        if self.shape_completion:
            Y1_cleared = [clear_border(lbl) for lbl in Y1]
            Y2_cleared = [clear_border(lbl) for lbl in Y2]
            _dist     = np.stack([star_dist(lbl,self.n_rays,mode=self.sd_mode)[self.b+(slice(None),)] for lbl in Y1_cleared])
            dist1      = _dist[self.ss_grid]
            dist_mask1 = np.stack([edt_prob(lbl[self.b][self.ss_grid[1:3]]) for lbl in Y1_cleared])

            _dist     = np.stack([star_dist(lbl,self.n_rays,mode=self.sd_mode)[self.b+(slice(None),)] for lbl in Y2_cleared])
            dist2      = _dist[self.ss_grid]
            dist_mask2 = np.stack([edt_prob(lbl[self.b][self.ss_grid[1:3]]) for lbl in Y2_cleared])
        else:
            # directly subsample with grid
            dist1      = np.stack([star_dist(lbl,self.n_rays,mode=self.sd_mode, grid=self.grid) for lbl in Y1])
            dist_mask1 = prob1

            dist2      = np.stack([star_dist(lbl,self.n_rays,mode=self.sd_mode, grid=self.grid) for lbl in Y2])
            dist_mask2 = prob2
        ...
        prob1 = np.expand_dims(prob1,-1)
        dist_mask1 = np.expand_dims(dist_mask1,-1)

        prob2 = np.expand_dims(prob2,-1)
        dist_mask2 = np.expand_dims(dist_mask2,-1)

        dist_and_mask1 = np.empty(dist1.shape[:-1]+(self.n_rays+1,), np.float32)
        dist_and_mask1[...,:-1] = dist1
        dist_and_mask1[...,-1:] = dist_mask1

        dist_and_mask2 = np.empty(dist2.shape[:-1]+(self.n_rays+1,), np.float32)
        dist_and_mask2[...,:-1] = dist2
        dist_and_mask2[...,-1:] = dist_mask2


        if self.n_classes is None:
            return ({'input': X}, 
                    {
                        'dist1': dist_and_mask1,
                        'prob1': prob1,
                        'dist2': dist_and_mask2,
                        'prob2': prob2
                    }   
                )

        else:
            prob_class1 = np.stack(tuple((mask_to_categorical(y, self.n_classes, self.classes[k]) for y,k in zip(Y1, idx))))
            prob_class2 = np.stack(tuple((mask_to_categorical(y, self.n_classes, self.classes[k]) for y,k in zip(Y2, idx))))

            prob_class1 = zoom(prob_class1, (1,)+tuple(1/g for g in self.grid)+(1,), order=0)
            prob_class2 = zoom(prob_class2, (1,)+tuple(1/g for g in self.grid)+(1,), order=0)
...
class Config2D(BaseConfig):
    ...
    def __init__(self, axes='YX', n_rays=32, n_channel_in=1, grid=(1,1), n_classes=None, backbone='unet', **kwargs):
        ...
        if backend_channels_last():
            ...
            self.net_mask1_shape        = None,None,1
            self.net_mask2_shape        = None,None,1
        else:
            self.net_input_shape       = self.n_channel_in,None,None
            self.net_mask1_shape        = 1,None,None
            self.net_mask2_shape        = 1,None,None

        self.train_shape_completion    = False
        self.train_completion_crop     = 32
        self.train_patch_size          = 256,256
        self.train_background_reg      = 1e-4 #lambda_2
        self.train_foreground_only     = 0.9
        self.train_sample_cache        = True

        self.train_dist_loss           = 'mae'
        self.train_wbr_loss            = False
        self.wbr_loss_weight           = 1 #lambda_3
        self.train_loss_weights        = ((1,0.2),(1,0.2)) if self.n_classes is None else ((1,0.2,1),(1,0.2,1)) #last or middle value is lambda_1
        self.train_class_weights       = ((1,1),(1,1)) if self.n_classes is None else ((1,)*(self.n_classes+1),(1,)*(self.n_classes+1))
        ...
        self.update_parameters(True, **kwargs)

        if not len(self.train_loss_weights) == 2 and len(self.train_loss_weights[0]) == (2 if self.n_classes is None else 3):
        ...
        if not len(self.train_class_weights) == 2 and len(self.train_class_weights[0]) == (2 if self.n_classes is None else 2*(self.n_classes+1)):
...
class StarDist2D(StarDistBase):
    ...
    def _build(self):
        ...
        unet_encoder, skips = unet_down_block(**unet_kwargs)(pooled_img)
        unet_decoder1 = unet_up_block(branch=1,**unet_kwargs)(layer=unet_encoder,skip_layers=skips)
        unet_decoder2 = unet_up_block(branch=2,**unet_kwargs)(layer=unet_encoder,skip_layers=skips)

        if self.config.net_conv_after_unet > 0:
            unet_branch1 = Conv2D(self.config.net_conv_after_unet, self.config.unet_kernel_size,
                          name='features1', padding='same', activation=self.config.unet_activation)(unet_decoder1)
            unet_branch2 = Conv2D(self.config.net_conv_after_unet, self.config.unet_kernel_size,
                          name='features2', padding='same', activation=self.config.unet_activation)(unet_decoder2)
        else:
            unet_branch1, unet_branch2 = unet_decoder1, unet_decoder2 

        output_prob1 = Conv2D(                 1, (1,1), name='prob1', padding='same', activation='sigmoid')(unet_branch1)
        output_dist1 = Conv2D(self.config.n_rays, (1,1), name='dist1', padding='same', activation='linear')(unet_branch1)

        output_prob2 = Conv2D(                 1, (1,1), name='prob2', padding='same', activation='sigmoid')(unet_branch2)
        output_dist2 = Conv2D(self.config.n_rays, (1,1), name='dist2', padding='same', activation='linear')(unet_branch2)

        if self.config.train_wbr_loss:
            concat = Concatenate(axis = 1,name='Concatenated_prob')([output_prob1, output_prob2])
            self.activity_reg = ActivityRegularizationLayer(wbr_loss_weight=self.config.wbr_loss_weight)
            _, _ = self.activity_reg(concat, input_img[:,:,1:])
        else:
            pass
        ...
        if self._is_multiclass():
            if self.config.net_conv_after_unet > 0:
                unet_class_branch1  = Conv2D(self.config.net_conv_after_unet, self.config.unet_kernel_size,
                                     name='features_class1', padding='same', activation=self.config.unet_activation)(unet_branch1)
                unet_class_branch2  = Conv2D(self.config.net_conv_after_unet, self.config.unet_kernel_size,
                                     name='features_class2', padding='same', activation=self.config.unet_activation)(unet_branch2)
            else:
                unet_class_branch1, unet_class_branch2  = unet_decoder1, unet_decoder2 

            output_prob_class1  = Conv2D(self.config.n_classes+1, (1,1), name='prob_class1', padding='same', activation='softmax')(unet_class_branch1)
            output_prob_class2  = Conv2D(self.config.n_classes+1, (1,1), name='prob_class2', padding='same', activation='softmax')(unet_class_branch2)
            return Model(
                inputs = input_img,
                outputs = {
                        'dist1': output_dist1,
                        'prob1': output_prob1,
                        'prob_class1': output_prob_class1,
                        'dist2': output_dist2,
                        'prob2': output_prob2,
                        'prob_class2': output_prob_class2
                        }
                )
        else:
            return Model(
    ...
    def train(self, X, Y1, Y2, validation_data, classes='auto', augmenter=None, seed=None, epochs=None, steps_per_epoch=None, workers=1):
        ...
        if self._is_multiclass() and len(validation_data) == 3:
            validation_data = tuple(validation_data) + ('auto',)
        ((len(validation_data) == (4 if self._is_multiclass() else 3))
            or _raise(ValueError(f'len(validation_data) = {len(validation_data)}, but should be {4 if self._is_multiclass() else 3}')))
        ...
        _data_val = StarDistData2D(validation_data[0],validation_data[1], validation_data[2], classes=classes_val, batch_size=n_take, length=1, **data_kwargs)
        data_val = _data_val[0]

        # expose data generator as member for general diagnostics
        self.data_train = StarDistData2D(X, Y1, Y2, classes=classes, batch_size=self.config.train_batch_size,
        ...
        if self.config.train_tensorboard:
            ...
            output_slices = [[slice(None)]*4,[slice(None)]*4,[slice(None)]*4,[slice(None)]*4]
            output_slices[3][1+channel] = slice(0,(self.config.n_rays//_n)*_n, self.config.n_rays//_n)
            if self._is_multiclass():

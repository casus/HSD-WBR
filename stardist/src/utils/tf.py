...
class CARETensorBoard(Callback):
    ...
    def set_model(self, model):
        ...
        image_for_outputs1 = np.arange(n_outputs) if self.image_for_outputs is None else self.image_for_outputs
        image_for_outputs2 = np.arange(n_outputs) if self.image_for_outputs is None else self.image_for_outputs

        output_target_shapes1 = [None]*n_outputs if self.output_target_shapes is None else self.output_target_shapes
        output_target_shapes2 = [None]*n_outputs if self.output_target_shapes is None else self.output_target_shapes
        self.gt_outputs1 = [K.placeholder(shape=_gt_shape(K.int_shape(x),sh)) for x,sh in zip(self.model.outputs,output_target_shapes1)]
        self.gt_outputs2 = [K.placeholder(shape=_gt_shape(K.int_shape(x),sh)) for x,sh in zip(self.model.outputs,output_target_shapes2)]

        input_slices  = (slice(None),) if self.input_slices  is None else self.input_slices
        output_slices1 = (slice(None),) if self.output_slices is None else self.output_slices
        output_slices2 = (slice(None),) if self.output_slices is None else self.output_slices
        if isinstance(input_slices[0],slice): # apply same slices to all inputs
            input_slices = [input_slices]*len(image_for_inputs)
        if isinstance(output_slices1[0],slice): # apply same slices to all outputs
            output_slices1 = [output_slices1]*len(image_for_outputs1)
            output_slices2 = [output_slices2]*len(image_for_outputs2)
        len(input_slices)  == len(image_for_inputs)  or _raise(ValueError())
        len(output_slices1) == len(output_slices2) == len(image_for_outputs1) == len(image_for_outputs2) or _raise(ValueError())
        ...
        # outputs
        for i1,i2,sl1,sl2 in zip(image_for_outputs1,image_for_outputs2,output_slices1,output_slices2):
            output_shape = self.model.output_shape if n_outputs==1 else self.model.output_shape[i1]
            # target
            output_layer1 = tf_normalize_layer(self.gt_outputs1[i1][tuple(sl1)])
            output_layer2 = tf_normalize_layer(self.gt_outputs2[i2][tuple(sl2)])
            layer_name1 = _name('net_target1', self.model.outputs[i1][0], i1, n_outputs)
            layer_name2 = _name('net_target2', self.model.outputs[i2][1], i2, n_outputs)
            tf_sums.append(tf.summary.image(layer_name1, output_layer1, max_outputs=self.n_images))
            tf_sums.append(tf.summary.image(layer_name2, output_layer2, max_outputs=self.n_images))
            # prediction
            ...
            if self.prob_out: # first half of output channels is mean, second half scale
                ...
                sep = sep // 2
            output_layer1 = tf_normalize_layer(self.model.outputs[i1][0][...,:sep][tuple(sl1)])
            output_layer2 = tf_normalize_layer(self.model.outputs[i2][1][...,:sep][tuple(sl2)])
            if self.prob_out:
                scale_layer1 = tf_normalize_layer(self.model.outputs[i1][0][...,sep:][tuple(sl1)], pmin=0, pmax=100)
                mean_name1  = _name('net_output_mean1',  self.model.outputs[i1[0]], i1, n_outputs)
                scale_name1 = _name('net_output_scale1', self.model.outputs[i1][0], i1, n_outputs)
                scale_layer2 = tf_normalize_layer(self.model.outputs[i2][1][...,sep:][tuple(sl2)], pmin=0, pmax=100)
                mean_name2  = _name('net_output_mean2',  self.model.outputs[i2[1]], i2, n_outputs)
                scale_name2 = _name('net_output_scale2', self.model.outputs[i2][1], i2, n_outputs)
                tf_sums.append(tf.summary.image(mean_name1, output_layer1, max_outputs=self.n_images))
                tf_sums.append(tf.summary.image(scale_name1, scale_layer1, max_outputs=self.n_images))
                tf_sums.append(tf.summary.image(mean_name2, output_layer2, max_outputs=self.n_images))
                tf_sums.append(tf.summary.image(scale_name2, scale_layer2, max_outputs=self.n_images))
            else:
                layer_name1 = _name('net_output1', self.model.outputs[i1][0], i1, n_outputs)
                layer_name2 = _name('net_output2', self.model.outputs[i2][0], i2, n_outputs)
        ...
        if self.validation_data and self.freq:
            if epoch % self.freq == 0:
                ...
                if self.model.uses_learning_phase:
                    ...
                    val_data = list(v[:self.n_images] for v in self.validation_data[:-2])
...
class CARETensorBoardImage(Callback):
    ...
    def __init__(self, model, data, log_dir,
        ...
        if isinstance(data,(list,tuple)):
            X = data[0]['input']
            Y1 = itemgetter('dist1','prob1')(data[1])
            Y2 = itemgetter('dist2','prob2')(data[1])
        else:
            X = data[0]['input']
            Y1 = itemgetter('dist1','prob1')(data[1])
            Y2 = itemgetter('dist2','prob2')(data[1])
        if self.n_inputs  == 1 and isinstance(X,np.ndarray): X = (X,)
        if self.n_outputs == 1 and isinstance(Y1,np.ndarray) and isinstance(Y2,np.ndarray): 
            Y1 = (Y1,)
            Y2 = (Y2,)
        
        (self.n_inputs == len(X) and self.n_outputs/2 == len(Y1) == len(Y2)) or _raise(ValueError())
        self.X = [v[:n_images] for v in X]
        self.Y1 = [v[:n_images] for v in Y1]
        self.Y2 = [v[:n_images] for v in Y2]
        ...
        image_for_outputs1 = np.arange(self.n_outputs/2) if image_for_outputs is None else image_for_outputs
        image_for_outputs2 = np.arange(self.n_outputs/2) if image_for_outputs is None else image_for_outputs

        input_slices  = (slice(None),) if input_slices  is None else input_slices
        output_slices1 = (slice(None),) if output_slices is None else output_slices[:2]
        output_slices2 = (slice(None),) if output_slices is None else output_slices[2:]
        if isinstance(input_slices[0],slice): # apply same slices to all inputs
            input_slices = [input_slices]*len(image_for_inputs)
        if isinstance(output_slices1[0],slice): # apply same slices to all outputs
            output_slices1 = [output_slices1]*len(image_for_outputs1)
        ...
        self.image_for_inputs = image_for_inputs
        self.image_for_outputs1 = image_for_outputs1
        self.image_for_outputs2 = image_for_outputs2
        self.input_slices = input_slices
        self.output_slices1 = output_slices1
        self.output_slices2 = output_slices2
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        # outputs
        temp = self.model.predict(self.X, batch_size=self.batch_size)
        Y1hat = (temp['prob1'], temp['dist1'])
        Y2hat = (temp['prob2'], temp['dist2'])

        if self.n_outputs == 1 and isinstance(Y1hat,np.ndarray) and isinstance(Y2hat, np.ndarray): 
            Y1hat = (Y1hat,)
            Y2hat = (Y2hat,)
        with self.file_writer.as_default():
            for i1,i2,sl1,sl2 in zip(self.image_for_outputs1, self.image_for_outputs2,
            self.output_slices1, self.output_slices2):
                i1 = int(i1)
                i2 = int(i2)
                output_shape = self.model.output_shape if self.n_outputs==4 else self.model.output_shape[i1]
            
                n_channels_out = sep = len(output_shape)
                if self.prob_out: # first half of output channels is mean, second half scale
                    n_channels_out % 2 == 0 or _raise(ValueError())
                    sep = sep // 2
                output_image1 = self._normalize_image(Y1hat[i1][...,:sep][tuple(sl1)])
                output_image2 = self._normalize_image(Y2hat[i2][...,:sep][tuple(sl2)])
                if self.prob_out:
                    scale_image1 = self._normalize_image(Y1hat[i1][...,sep:][tuple(sl1)], pmin=0, pmax=100)
                    scale_name1 = self._name('net_output_scale1', self.model.outputs[i1][0], i1, self.n_outputs)
                    output_name1 = self._name('net_output_mean1',  self.model.outputs[i1][0], i1, self.n_outputs)
                    scale_image2 = self._normalize_image(Y2hat[i2][...,sep:][tuple(sl2)], pmin=0, pmax=100)
                    scale_name2 = self._name('net_output_scale2', self.model.outputs[i2][1], i2, self.n_outputs)
                    output_name2 = self._name('net_output_mean2',  self.model.outputs[i2][1], i2, self.n_outputs)
                    tf.summary.image(output_name1, output_image1, step=epoch)
                    tf.summary.image(scale_name1, scale_image1, step=epoch)
                    tf.summary.image(output_name2, output_image2, step=epoch)
                    tf.summary.image(scale_name2, scale_image2, step=epoch)
                else:
                    output_name1 = self._name('net_output1', self.model.outputs[i1][0], i1, self.n_outputs)
                    output_name2 = self._name('net_output2', self.model.outputs[i2][1], i2, self.n_outputs)
                    tf.summary.image(output_name1, output_image1, step=epoch)
                    tf.summary.image(output_name2, output_image2, step=epoch)
        ...
        if epoch == 0:
            with self.file_writer.as_default():
                ...
                self.output_slices1,self.output_slices2):
                    i1 = int(i1)
                    i2 = int(i2)
                    target_name1 = self._name('net_target1', self.model.outputs[i1][0], i1, self.n_outputs)
                    target_image1 = self._normalize_image(self.Y1[i1][tuple(sl1)])
                    target_name2 = self._name('net_target2', self.model.outputs[i2][1], i2, self.n_outputs)
                    target_image2 = self._normalize_image(self.Y2[i2][tuple(sl2)])
                    tf.summary.image(target_name1, target_image1, step=epoch)
                    tf.summary.image(target_name2, target_image2, step=epoch)